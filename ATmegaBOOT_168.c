...
/*#define MAX_TIME_COUNT UINT32_MAX */
...
int main(void)
{
	uint8_t ch,ch2;
	uint16_t w;
	if (eeprom_read_byte((void *)E2END)!=0xff) { // if last eeprom value is not 0xff skip straight to the code. Change last eeprom value if u do not want to stay in bootloader.
